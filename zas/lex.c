...
#else
/* A palavra-chave 'inline' foi removida para compatibilidade com C89 */
...
static void openFile(char *name)
{
    char *s;
    register FILE *fp;
    char fullPath[256]; /* A buffer to construct the full path */

    if ((fp = fopen(name, "r")) == 0) { /* Try opening directly */
        if (includePath != NULL) { /* If an include path is specified */
            sprintf(fullPath, "%s/%s", includePath, name);
