...
#else
#ifdef __STDC__
void err_handling(const char *fmt, va_list args); /* safer version */
_Noreturn void error(const char *fmt, ...);       /* safer version */
#else
void err_handling(); /*  ok++ works good	*/
void error();        /*  ok++ works good	*/
#endif
...
struct aaa
{
    const char *name;  /* Record type name  		*/
...
unsigned char order32[] = {0, 1, 2, 3}; /* 32 bit order */
...
int main(int argc, char **argv)
{
    ...
    if (argc == 1)
    {                          /* When starting the program without     */
        *(argv + 1) = (char *)"l.obj"; /* parameters, process the file "l.obj"  */
        ++argc;
    }
    next_arg = argv + 1;
...
int chk_rec()
{
    ...
    */
    ...
    if (fread(buf_data, 1, (size_t)rec_len, stdin) != (size_t)rec_len)
...
void cod_sym()
{
    ...
    while (l1 < &buf_data[rec_len])
    {
        ...
        l4 = calc_val(l1 + 4);
#if CPM
        printf("\t\t%s\t%s\t%D", l3, (*l2 != 0) ? (char *)l2 : (((l4 & 6) != 0) ? "" : "(abs)"), calc_long(l1));
#else
...
#else
void err_handling(const char *fmt, va_list args)
{
