 ...
 * =============================================================================
 * Versão final corrigida para compilar sem erros ou alertas no
 * compilador Hi-Tech C com suporte parcial ao padrão ANSI C89/C90.
 * =============================================================================
 */
...
/* 'const' não é suportado, substituído por #define */
#define PI 3.14159f

/* Protótipos de funções no estilo ANSI C */
...
void demonstrate_variables_and_types(void)
{
    ...
    unsigned unsigned_var = 200; /* Sufixo 'U' removido */
    float float_var = 3.14f;
    double double_var = 2.71828;
    /* 'long double' não é suportado */

    /* 'volatile' não é suportado, declarada como int normal */
    int sensor_reading;
    sensor_reading = 10;
...
void demonstrate_operators(void)
{
    ...
    printf("Aritmeticos: a + b = %d, a * b = %d, a / b = %d, a %% b = %d\n", a + b, a * b, a / b, a % b);
    c = 5;
    printf("Unarios: c++ = %d, ++c = %d\n", c++, ++c);
    c = 10;
    c += 5;
...
void demonstrate_arrays_and_strings(void)
{
    /* Arrays locais devem ser 'static' para serem inicializados na declaração */
    static int simple_array[] = {10, 20, 30, 40, 50};
    static int matrix[2][3] = {{1, 2, 3}, {4, 5, 6}};
    static char str1[] = "Primeira string";
    char str2[20];
    int i, j;

    printf("\n--- 6. Arrays e Strings ---\n");
    printf("String declarada e inicializada: %s\n", str1); /* Uso de str1 para evitar alerta */
    printf("Terceiro elemento do array simples: %d\n", simple_array[2]);

    strcpy(str2, "Copia");
    printf("String copiada (strcpy): %s\n", str2);
    // if (isupper(str2[0])) {
    //     printf("O primeiro caractere de '%s' e maiusculo.\n", str2);
    // }

    if (my_isupper(str2[0]))
    {
        printf("O primeiro caractere de '%s' e maiusculo.\n", str2);
    }
...
void demonstrate_structs_unions_enums(void)
{
    ...
    status.active = 1;
    status.mode = 2;
    printf("Struct com Bit-Fields: Ativo=%u, Modo=%u\n", status.active, status.mode);
...
#ifdef __STDC__
    printf("O compilador se identifica como compativel com o padrao C.\n");
#endif

    /* __DATE__ e __TIME__ não são suportados */
...
void demonstrate_file_io(void)
{
    FILE *file_ptr;
    char buffer[100];
    /* Array local deve ser 'static' para ser inicializado */
    static int numbers[] = {1, 2, 3, 4, 5};
    int read_numbers[5];

    printf("\n--- 10. Entrada e Saida de Arquivos (File I/O) ---\n");

    /* -- Escrita e Leitura de Arquivo Texto -- */
    file_ptr = fopen("texto.txt", "w");
    if (file_ptr == NULL)
    {
        printf("Erro ao abrir texto.txt para escrita!\n"); // Substituído fprintf(stderr)
        return;
    }
    ...
    if (file_ptr == NULL)
    {
        printf("Erro ao abrir texto.txt para leitura!\n"); // Substituído fprintf(stderr)
        return;
    }
    ...
    if (file_ptr == NULL)
    {
        printf("Erro ao abrir dados.bin para escrita!\n"); // Substituído fprintf(stderr)
        return;
    }
