...
expr_t *parseConstExpr(uint8_t n)
{
    ...
    byte_9d37 = n;
    if ((st = parseExpr(T_EROOT, sub_0bfc(), 0)))
    { /* PMO added dummy arg3 */
        if (!isConstExpr(st))
...
expr_t *parseExpr(uint8_t p1, register expr_t *lhs, expr_t *rhs)
{
    ...
    { /* 1740 */
        ...
        rhs = sub_1ccc(parseExpr(T_STAR, savedLhs, sub_1ebd(lhs)),
             ...
             (!hasRhs ||
              ((rhs->attr.indirection & 1) && rhs->attr.nodeType == SYMNODE)))
    { /* 17ab */
    ...
    else if (operatorFlags & OP_VOIDFUNC)
    { /* 1937 */
