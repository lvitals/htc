...
uint8_t yylex(void) {
    ...
    for (;;) {
        ...
        case '/':
            ch = getCh(); /* Read the next character after '/'*/
            if (ch == '*') { /* Start of a C-style comment*/
                for (;;) {
                    ch = getCh();
                    if (ch == '*') {
                        ch = getCh();
                        if (ch == '/') {
                            ...
                            ungetCh = ch; /* Unget the character if it's not '/'*/
                        }
                    }
                    if (ch == EOF) {
                        fatalErr("EOF in comment");
                    }
                }
                continue; /* Continue the main yylex loop to get the next token*/
            } else if (ch == '=') {
                return P1_EQDIV;
            }
            ungetCh = ch;
...
uint8_t parseNumber(int16_t ch) {
    ...
    if (ch == 'e' || ch == 'E') {
        isFloat = true; /* Ensure it's treated as float if it has exponent */
        *s++ = (char)ch; /* Add 'e' or 'E' to buffer */
        ch = getCh();
        if (ch == '+' || ch == '-') {
            *s++ = (char)ch;
            ch = getCh();
    ...
    if ((ch == 'f' || ch == 'F') && isFloat) {
        ch = getCh();  /* Consume the 'f' */
    }
    
    if (isFloat) {
        ungetCh = ch;
        *s = 0;
        yylval.yStr = xalloc(s - buf + 1);
