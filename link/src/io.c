...
static void writeText()
{
    ...
    if (optC)
    {
        ...
        if (fwrite(curAddr, 1, writeLen, outFp) != (size_t)writeLen)
            writeErr();
    }
    else
    {
        ...
        if (fwrite(tmp, RECORD_HDR, 1, outFp) != 1 ||
            fwrite(textRecBuf, 1, textHdrLen, outFp) != (size_t)textHdrLen ||
            fwrite(curAddr, 1, writeLen, outFp) != (size_t)writeLen)
            writeErr();
    }
...
void wrRecord()
{
    ...
    if (fwrite(nonTextRecBuf, 1, len, outFp) != (size_t)len)
        writeErr();
